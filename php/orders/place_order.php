...
// verify order items list is valid
...
$stmt = $conn->prepare(
    'INSERT INTO orders (order_id, customer_email, customer_name, address, city, items_json, total, payment_method)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
);
$stmt->bind_param('ssssssds', $orderId, $email, $name, $address, $city, $itemsJson, $totalFloat, $paymentMethod);

if ($stmt->execute()) {
    // clear persistent cart records for logged-in user
